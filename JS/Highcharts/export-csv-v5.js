    ...
    Highcharts.Chart.prototype.getDataRows = function () {
        ...
        each(this.series, function (series) {
            var keys = series.options.keys,
                ...
                requireSorting = true, //series.requireSorting,
                categoryMap = {},
            ...
            if (series.options.includeInCSVExport !== false && series.visible !== false) { // #55
                ...
                each(series.options.data, function (point, pIdx) {
                    var xValue = pick(point.x, point[0], pIdx),
                        yValue = pick(point.y, point[1]),
                        zValue = pick(point.x, point[2]), // add support for three axes charts, eg ternary
                        pointArrayMapValues = [yValue, zValue],
                        key = requireSorting ? xValue : pIdx + 1,
                        prop,
                        val;
                    ...
                    rows[key].x = xValue;
                    rows[key].xValues[xAxisIndex] = xValue;
                    ...
                    while (j < valueCount) {
                        ...
                        val = point[prop];
                        rows[key][i + j] = pick(categoryMap[prop][val], pointArrayMapValues[j], yValue); // Pick a Y axis category if present
                        j = j + 1;
                    }
